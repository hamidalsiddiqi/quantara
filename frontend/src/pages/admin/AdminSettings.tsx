...
import { Loader2, Settings, Save, Megaphone, Plus } from 'lucide-react';
...
export default function AdminSettings() {
    ...
    const createAnnMutation = useMutation({
        mutationFn: () => api.admin.createAnnouncement({ title: annTitle, message: annMessage }),
        onSuccess: () => {
        ...
        onError: (e: any) => toast({ title: e.message ?? 'Failed to post announcement', variant: 'destructive' }),
    });
                ...
                <>
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Create a new announcement that will be displayed in the notifications feed on all user dashboards.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="space-y-1.5">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Textarea
                                    ...
                                    rows={3}
                                    value={annMessage}
                                    onChange={(e: any) => setAnnMessage(e.target.value)}
                                />
                            </div>
                            <div className="flex justify-end">
                                <Button
                                    variant="brand"
                                    disabled={!annTitle || !annMessage || createAnnMutation.isPending}
                                    onClick={() => createAnnMutation.mutate()}
                                    className="gap-2"
                                >
                                    {createAnnMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}
                                    Post Announcement
                                </Button>
                            </div>
                        </CardContent>
                                ...
                                <div className="space-y-2">
                                    {data.settings.map((s) => (
                                        <div key={s.key} className="flex items-start gap-3 rounded-lg bg-secondary/30 p-3 text-sm">
                                            <code className="text-cyan-400 flex-shrink-0 font-mono text-xs">{s.key}</code>
