...
function intervalsBetween(a: Date, b: Date): number {
  ...
  return Math.floor(ms / (60 * 1000)); // 1 min = 60,000 ms
}
...
async function processCycle(cycleId: string): Promise<void> {
  await prisma.$transaction(async (tx) => {
    ...
    for (let day = cycle.daysAccrued; day < dueDays; day++) {
      const accruedOn = utcMinute(new Date(cycle.startedAt.getTime() + (day + 1) * 60 * 1000));
    ...
    if (fresh && fresh.daysAccrued >= fresh.durationDays && fresh.status === 'ACTIVE') {
      ...
      const releaseDay = utcMinute(new Date(fresh.startedAt.getTime() + (fresh.durationDays + 1) * 60 * 1000));
      try {
        await tx.earning.create({
          data: {
            userId: fresh.userId,
